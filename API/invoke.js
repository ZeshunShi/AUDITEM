...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        if (fcn === "createRecord") {
            ...
            message = `Successfully added the Integrity_Record with key ${args[0]}`
        } else if (fcn == "createPrivateKeyStorage") {
            console.log(`Transient data is : ${transientData}`)
            let transData = JSON.parse(transientData)
            console.log(`records is : ${JSON.stringify(transData)}`)
            let key = Object.keys(transData)[0]
            const transientDataBuffer = {}
            transientDataBuffer[key] = Buffer.from(JSON.stringify(transData.keys))
